...
test('a planned block can be moved before another block for the same employee',async t=>{
 const f=await fullApp(t),w=f.w;
 vm.runInContext("state.orders.push({id:'drag-a',orderNo:'DRAG-A',product:'Eerste',qty:1,active:true,deadline:'2026-10-20'},{id:'drag-b',orderNo:'DRAG-B',product:'Tweede',qty:1,active:true,deadline:'2026-10-20'},{id:'drag-shelby',orderNo:'SHELBY',product:'Shelby',qty:80,active:true,deadline:'2026-10-20'});state.tasks.push({id:'drag-first',orderId:'drag-a',seq:1,name:'Eerste taak',machine:'Reichenbacher',employee:'Ralph',estimate:60,status:'open',lockedPlanning:true,date:'2026-09-24',start:'08:15',planSegments:[{date:'2026-09-24',employee:'Ralph',start:'08:15',minutes:60}]},{id:'drag-second',orderId:'drag-b',seq:1,name:'Tweede taak',machine:'Reichenbacher',employee:'Ralph',estimate:60,status:'open',lockedPlanning:true,date:'2026-09-24',start:'09:15',planSegments:[{date:'2026-09-24',employee:'Ralph',start:'09:15',minutes:60}]},{id:'drag-shelby-task',orderId:'drag-shelby',seq:1,name:'Shelby instellen',machine:'Teach-in',employee:'Ralph',estimate:45,status:'open',lockedPlanning:true,date:'2026-09-24',start:'10:15',planSegments:[{date:'2026-09-24',employee:'Ralph',start:'10:15',minutes:45}]})",f.ctx);w.RALAB_PERFORMANCE.invalidate();
 const result=w.RALAB_MANUAL_START.reorderBefore('drag-shelby-task','drag-first','2026-09-24','Ralph');assert.equal(result.error,undefined);const tasks=f.state().tasks;assert.equal(tasks.find(x=>x.id==='drag-shelby-task').start,'08:15');assert.ok(tasks.find(x=>x.id==='drag-first').start>'08:15');assert.ok(result.moved>=2);assert.deepEqual(f.errors,[]);
});
