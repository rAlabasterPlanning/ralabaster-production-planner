...
test('an existing quotation reopens in calculation and updates without creating a duplicate',async t=>{
 ...
 d.querySelector('.navbtn[data-view="quotes"]').click();await f.wait(180);const quoteRow=d.querySelector('[data-open-quote-row="20260923-77"]');assert.ok(quoteRow);quoteRow.click();await f.wait(60);assert.match(d.querySelector('#modalRoot').textContent,/Customer details/);
