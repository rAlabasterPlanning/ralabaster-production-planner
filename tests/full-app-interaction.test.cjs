...
test('clicking a planned task shows deadline, current finish and next task planning',async t=>{
 ...
 const card=d.querySelector('[data-task="context-a"].week-task-compact');assert.ok(card);assert.ok(card.querySelector('.week-task-icon svg'));assert.equal(card.querySelector('.workplace-number').textContent,'1');assert.equal(card.querySelector('.week-task-order').textContent,'10x Context product');assert.equal(card.querySelector('.op'),null);assert.doesNotMatch(card.textContent,/CTX-01/);card.click();await f.wait(40);
