...
function saveQuoteDirect(){
 try{
  ...
  products.forEach((p,i)=>{const c=costFor(p,p.qty),guide=autoSale(p,c.unit),sale=finalSale(p,i,c.unit),margin=sale-c.unit,priceTiers=window.RALAB_CALC_PRICE?.quoteTiers?.(i,p)||[];s.quotes.push({id:'q_'+stamp+'_'+i,quoteNo,quoteGroupId:groupId,isMultiProductLine:products.length>1,lineNo:i+1,customerId,customerReference,project:val('cProject'),orderNo:quoteNo,deadline:val('cDeadline'),name:p.name,qty:p.qty,ops:structuredClone(p.ops),materialCost:p.materialCost,materialMode:p.materialMode,marginMode:p.marginMode,marginValue:p.marginValue,costUnit:c.unit,calculatedSaleUnit:guide,manualSalePrice:sale===guide?null:sale,saleUnit:sale,total:sale*p.qty,includeTierPricing:priceTiers.length>0,priceTiers:structuredClone(priceTiers),grossMarginUnit:margin,grossMarginPct:sale?margin/sale*100:0,markupPct:c.unit?margin/c.unit*100:0,status:'concept',created:new Date().toISOString().slice(0,10),isTestOrder:test})});
  if(s.quotes.length<=before)throw new Error('Er is geen offerte toegevoegd.');
  persist();openQuotes();return true;
 }catch(e){console.error('Offerte opslaan mislukt',e);alert('Offerte opslaan mislukt: '+(e?.message||e));return false}
}
function quoteButton(){return [...(document.querySelectorAll('#calcFollowupActions button, #view-calculation button')||[])].find(b=>(b.textContent||'').trim().includes('Offerte opslaan'))||null}
...
function install(){ensureCustomerReference();const calc=window.RALAB_CALC;if(!calc)return false;calc.saveQuote=saveQuoteDirect;if(!calc.__quoteNoNewWrapped){const oldNew=calc.newCalc;calc.newCalc=function(){const r=oldNew.apply(this,arguments),pricing=window.RALAB_CALC_PRICE;if(pricing){pricing.manualByIndex=[];pricing.tiersByIndex=[];pricing.includeTiersByIndex=[]}setTimeout(()=>{ensureCustomerReference();prefillQuoteNo(true);bindQuoteButton()},30);return r};calc.__quoteNoNewWrapped=true}bindQuoteButton();return true}
