...
function openCustomer(id){
 ...
 const rows=orders.map(o=>`<tr><td><button class="btn small" type="button" onclick="RALAB_ERP.openOrder('${o.id}')">${esc(o.orderNo||'')}</button></td><td>${esc(o.product||'')}</td><td>${Number(o.qty)||0}</td><td>${esc(status(o))}</td><td>${esc(o.communicatedDeadline||o.deadline||'—')}</td><td style="text-align:right">${euro(Number(o.totalSale)||((Number(o.saleUnit)||0)*(Number(o.qty)||0)))}</td></tr>`).join('');
 ...
 <h3 style="margin-top:18px">Orders</h3><div style="overflow:auto"><table><thead><tr><th>Order</th><th>Product</th><th>Aantal</th><th>Status</th><th>Deadline</th><th style="text-align:right">Waarde</th></tr></thead><tbody>${rows||'<tr><td colspan="6">Geen orders voor deze klant.</td></tr>'}</tbody></table></div>
...
function renderOrderOverview(){
 ...
 ${all.map(o=>{const f=orderFeasibility(o),t=currentStep(o);return `<button type="button" onclick="RALAB_ERP.openOrder('${o.id}')" style="text-align:left;padding:14px;border:2px solid;border-radius:14px;${palette[f.key]};cursor:pointer">
   ...
   <div style="margin-top:10px;font-size:13px"><b>Nu:</b> ${esc(t?.name||'—')}<br><b>Intern gereed:</b> ${esc(o.internalExpectedDate||'—')}<br><b>Deadline:</b> ${esc(o.communicatedDeadline||o.deadline||'—')}<br><b>Speling:</b> ${esc(f.detail)}</div>
