...
function applyProduct(p){
 ...
 for(const cb of rows){const id=cb.dataset.opcheck,name=cb.closest('tr')?.querySelector('td:nth-child(2) b')?.textContent?.trim(),op=(p.ops||[]).find(x=>x.id===id||x.name===name);cb.checked=!!op;const values=op?[['rate',op.rate],['mode',op.mode],['minutes',op.minutes],['eb',op.externalBatch||0],['eu',op.externalUnit||0]]:name==='Steen inkopen Spanje (extern)'?[['rate',0],['mode','external'],['minutes',50400],['eb',0],['eu',0]]:[];for(const [key,value] of values){const el=root.querySelector(`[data-${key}="${id}"]`);if(el)el.value=value}}
