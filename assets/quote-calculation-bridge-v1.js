...
function showCalc(no){
 ...
 if(window.RALAB_CALC_PRICE)window.RALAB_CALC_PRICE.manualByIndex=lines.map(q=>num(q.saleUnit));
...
function updateQuote(no){
 ...
 const updated=products.map((p,i)=>{const prior=old[i],cost=costFor(p),manual=price[i],sale=manual===null||manual===undefined||manual===''?autoSale(p,cost.unit):num(manual),margin=sale-cost.unit;return{...(prior||first),id:prior?.id||`q_${stamp}_${i}`,quoteNo:String(no),orderNo:String(no),quoteGroupId:group,lineNo:i+1,isMultiProductLine:products.length>1,customerId,customerReference:reference,project,name:p.name,qty:p.qty,templateId:p.templateId||'',ops:structuredClone(p.ops||[]),materialCost:num(p.materialCost),materialMode:p.materialMode||'unit',marginMode:p.marginMode||'factor',marginValue:num(p.marginValue),costUnit:cost.unit,calculatedSaleUnit:autoSale(p,cost.unit),manualSalePrice:sale,saleUnit:sale,total:sale*p.qty,estimatedReadyDate:prior?.estimatedReadyDate||deadline,deadline:prior?.deadline||deadline,grossMarginUnit:margin,grossMarginPct:sale?margin/sale*100:0,markupPct:cost.unit?margin/cost.unit*100:0,updatedAt:new Date().toISOString()}});
