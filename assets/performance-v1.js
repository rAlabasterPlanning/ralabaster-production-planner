...
// UI reads only indexed/lightweight planning data. Exact forecasts run only for explicit planning actions.
(()=>{
  ...
  let pendingViewFrame=0,cloudStamp='',exactMode=false;
  ...
  async function fetchPaged(makeQuery){const out=[];let from=0;while(true){const {data,error}=await makeQuery(from,from+PAGE_SIZE-1);if(error)throw error;const rows=data||[];out.push(...rows);if(rows.length<PAGE_SIZE)break;from+=PAGE_SIZE;if(from>200000)break}return out}
  ...
  async function loadNormalizedCloud(silent=false){
    ...
    try{
      ...
      const meta=metaRes.data?.data||{};state={...state,...meta,orders:orderRows.map(r=>r.data),tasks:taskRows.map(r=>r.data)};
      if(!Array.isArray(state.deletedTasks))state.deletedTasks=[];if(!Array.isArray(state.history))state.history=[];
      cloudStamp=metaRes.data?.updated_at||cloudStamp;normalizedReady=true;invalidate();seedHashes();
      const local=compactLocalState();if(local)try{localStorage.setItem(KEY,local)}catch(_){ }
      cloudStatus='online';if(!silent)render();renderOnlineBadge();return true;
    }catch(e){console.warn('Genormaliseerde plannerdata laden mislukt; snapshot blijft actief.',e);cloudStatus='error';renderOnlineBadge();return false}
  ...
  async function saveNormalizedCloud(){
    if(!supabaseClient||!cloudUser||cloudLoading)return;if(!normalizedReady)return window.__RALAB_ORIGINAL_SAVE_CLOUD_STATE?.();
    cloudStatus='syncing';renderOnlineBadge();const now=new Date().toISOString(),orders=state.orders||[],tasks=state.tasks||[];
    const activeByOrder=new Map(orders.map(o=>[o.id,o.active!==false&&o.status!=='completed'])),changedOrders=[],changedTasks=[];
    for(const o of orders){const h=hash(o);if(lastOrderHashes.get(o.id)!==h)changedOrders.push(orderRow(o,now))}
    for(const t of tasks){const h=hash(t);if(lastTaskHashes.get(t.id)!==h)changedTasks.push(taskRow(t,activeByOrder.get(t.orderId)!==false,now))}
    try{
      if(changedOrders.length)await upsertChunks('planner_orders_v2',changedOrders);if(changedTasks.length)await upsertChunks('planner_tasks_v2',changedTasks);
      const meta={...state,orders:[],tasks:[],normalizedVersion:2};const {error}=await supabaseClient.from('planner_shared_state').upsert({workspace_id:WORKSPACE_ID,data:meta,updated_at:now},{onConflict:'workspace_id'});if(error)throw error;
  ...
  function install(){
    ...
    // Never run a full forecast merely because a view or modal is opened.
    ...
    // Keep the exact deadline check for the explicit "Gehele order inplannen" action.
    ...
    save=function(){invalidate();const txt=compactLocalState();if(txt)try{localStorage.setItem(KEY,txt)}catch(_){ }scheduleCloudSave()};
    saveCloudState=saveNormalizedCloud;
    loadCloudState=async function(silent=false){if(!normalizedReady)return originalLoadCloudState(silent);if(!silent)return loadNormalizedCloud(false);if(!supabaseClient||!cloudUser||cloudLoading)return;try{const {data,error}=await supabaseClient.from('planner_shared_state').select('updated_at').eq('workspace_id',WORKSPACE_ID).maybeSingle();if(error)throw error;const next=data?.updated_at||'';if(next&&next===cloudStamp)return;return loadNormalizedCloud(true)}catch(e){console.warn(e)}};
  ...
  const normTimer=setInterval(async()=>{if(!installed||normalizedReady||normalizedInitBusy)return;if(typeof supabaseClient!=='undefined'&&supabaseClient&&typeof cloudUser!=='undefined'&&cloudUser){const ok=await loadNormalizedCloud(false);if(ok)clearInterval(normTimer)}},350);
})();
