...
function taskMinutes(o,t){const mode=taskMode(o,t),op=templateOp(o,t);if(op&&Number.isFinite(Number(op.minutes)))return Number(op.minutes);if(mode==='unit')return (Number(t.estimate)||0)/Math.max(1,Number(o.qty)||1);if(mode==='external')return (Number(t.externalLeadDays)||14)*1440;return Number(t.estimate)||0}
function taskRate(o,t){const op=templateOp(o,t);if(op&&Number.isFinite(Number(op.rate)))return Number(op.rate);if(Number.isFinite(Number(t.rate)))return Number(t.rate);return Number(rates[t.machine]??rates[t.name]??0)}
...
function renderEditor(){
 ...
 box.innerHTML=`<div class="toolbar"><div><h2 style="margin:0">Ordercalculatie · ${esc(o.orderNo||'')}</h2><div class="muted">Je bewerkt rechtstreeks de bestaande order <b>${esc(o.product||'')}</b>. Opslaan maakt géén nieuwe order.</div></div><div class="spacer"></div><button class="btn" type="button" data-oc-back>Terug naar order</button><button class="btn" type="button" data-oc-save>Opslaan naar order</button><button class="btn primary" type="button" data-oc-save-plan>Opslaan en direct inplannen</button></div>
...
function saveOrderCalc(openNext=false){const s=S(),o=orderById(editOrderId);if(!s||!o)return false;const qty=Math.max(1,Number(document.getElementById('ocQty')?.value)||1),customerId=document.getElementById('ocCustomer')?.value||'',customer=s.customers?.find(c=>c.id===customerId),ready=document.getElementById('ocReady')?.value||'';o.customerId=customerId;o.customerName=customer?.name||o.customerName||'';o.project=document.getElementById('ocProject')?.value.trim()||'';o.product=document.getElementById('ocProduct')?.value.trim()||o.product;o.qty=qty;o.saleUnit=Math.max(0,Number(document.getElementById('ocSale')?.value)||0);o.materialCostUnit=Math.max(0,Number(document.getElementById('ocMaterial')?.value)||0);if(ready){o.quotedEstimatedReadyDate=ready;o.communicatedDeadline=ready;o.maximumReadyDate=addDays(ready,14);o.internalTargetDate=addDays(o.maximumReadyDate,-3);o.deadline=o.maximumReadyDate;o.bufferModelVersion=2}
 ...
 try{window.RALAB_DEADLINE_PLANNER?.normalizeSequences?.();window.RALAB_DEADLINE_PLANNER?.planOrderStrict?.(o,{allowPeter:false,allowSaturday:false})}catch(e){console.warn('Order opnieuw plannen is overgeslagen',e)}
