...
function liveSummary(){
 ...
 const materialTotal=material*qty;let processingTotal=0,externalTotal=0,total=materialTotal;
 for(const r of document.querySelectorAll('#ocRows [data-oc-row]')){
   ...
   if(mode==='external'){externalTotal+=ext*qty;total+=ext*qty;continue}
   const totalMinutes=mode==='unit'?mins*qty:mins;
   const processing=totalMinutes/60*rate;processingTotal+=processing;total+=processing;
 }
 ...
 return {qty,materialTotal,processingTotal,externalTotal,costUnit,totalCost:total,saleUnit:sale,totalSale,marginUnit,marginTotal,marginPct};
}
function renderMarginSummary(){
 const x=liveSummary(),el=document.getElementById('ocMarginSummary');if(!x||!el)return;
 const marginColor=x.marginTotal<0?'#a83232':'#126c50';
 el.innerHTML=`<div class="grid3">
   <div><b>Inkoop / materiaal totaal</b><div style="font-size:20px">${fmtEuro(x.materialTotal)}</div></div>
   <div><b>Bewerkingen totaal</b><div style="font-size:20px">${fmtEuro(x.processingTotal)}</div></div>
   <div><b>Extern totaal</b><div style="font-size:20px">${fmtEuro(x.externalTotal)}</div></div>
   ...
   <div><b>Marge / product</b><div style="font-size:22px;color:${marginColor}">${fmtEuro(x.marginUnit)} <span>(${x.marginPct.toFixed(1).replace('.',',')}%)</span></div></div>
   ...
   <div><b>Totale marge order</b><div style="font-size:20px;color:${marginColor}">${fmtEuro(x.marginTotal)}</div></div>
 </div>`;
}
...
function renderEditor(){
 ...
 box.innerHTML=`<div class="toolbar"><div><h2 style="margin:0">Ordercalculatie · ${esc(o.orderNo||'')}</h2><div class="muted">Je bewerkt rechtstreeks de bestaande order <b>${esc(o.product||'')}</b>. Opslaan maakt géén nieuwe order.</div>${o.waitingMaterial?'<div style="margin-top:6px"><span class="badge" style="background:#fff1c7;color:#6b4d00">🟠 Wacht op materiaal</span></div>':''}</div><div class="spacer"></div><button class="btn" type="button" data-oc-back>Terug naar order</button><button class="btn" type="button" data-oc-save>Alleen opslaan</button><button class="btn" type="button" data-oc-material>${o.waitingMaterial?'Materiaal binnen':'Wacht op materiaal'}</button><button class="btn primary" type="button" data-oc-save-plan ${o.waitingMaterial?'disabled title="Eerst materiaal binnen melden"':''}>Opslaan en alleen deze order inplannen</button></div>
 ...
 <div id="ocMarginSummary" class="order-calc-margin-summary" aria-live="polite"></div>
 ...
 <div class="notice" style="margin-top:12px"><b>Duidelijk gescheiden:</b> <b>Alleen opslaan</b> zet deze order niet in de planning. De groene knop plant uitsluitend deze order; andere ongeplande orders blijven los staan.</div>`;
 root.prepend(box);renderMarginSummary();box.scrollIntoView?.({block:'start'});
}
function addRow(){const o=orderById(editOrderId),tbody=document.getElementById('ocRows');if(!o||!tbody)return;const t={id:uid(),name:'Nieuwe stap',machine:'',estimate:0,type:'internal',status:'open'};tbody.insertAdjacentHTML('beforeend',row(t,o,tbody.children.length));renderMarginSummary()}
...
function saveOrderCalc(openNext=false,shouldPlan=false){const s=S(),o=orderById(editOrderId);if(!s||!o)return false;o.planningCheckedAt='';const qty=Math.max(1,Number(document.getElementById('ocQty')?.value)||1),customerId=document.getElementById('ocCustomer')?.value||'',customer=s.customers?.find(c=>c.id===customerId),ready=document.getElementById('ocReady')?.value||'';o.customerId=customerId;o.customerName=customer?.name||o.customerName||'';o.project=document.getElementById('ocProject')?.value.trim()||'';o.product=document.getElementById('ocProduct')?.value.trim()||o.product;o.qty=qty;o.saleUnit=Math.max(0,Number(document.getElementById('ocSale')?.value)||0);o.materialCostUnit=Math.max(0,Number(document.getElementById('ocMaterial')?.value)||0);if(ready){o.quotedEstimatedReadyDate=ready;o.communicatedDeadline=ready;o.maximumReadyDate=addDays(ready,14);o.internalTargetDate=addDays(o.maximumReadyDate,-3);o.deadline=o.maximumReadyDate;o.bufferModelVersion=2}
 ...
 try{window.RALAB_PERFORMANCE?.invalidate?.()}catch(_){ }
 try{save()}catch(e){console.error(e)}
 if(shouldPlan&&!o.waitingMaterial){window.RALAB_ORDER_CONTROLS?.planAndCheck?.(o.id);return true}
 ...
 alert('Order en productiestappen zijn opgeslagen. Deze order is niet ingepland.');renderEditor();return true;
}
