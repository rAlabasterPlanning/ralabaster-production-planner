...
const FREEZE_DAYS=1; // today + tomorrow stay stable unless a task is manually moved
...
const machineKey=t=>String(t?.machine||t?.name||'').replace(/\s*[-–]?\s*instellen\b/ig,'').replace(/\s+/g,' ').trim().toLowerCase();
...
function normalizeSequences(){const s=S();if(!s)return;for(const o of s.orders||[]){if(isGeneral({orderId:o.id,isGeneralWork:o.isGeneralWork}))continue;const ts=(s.tasks||[]).filter(t=>t.orderId===o.id&&!isGeneral(t)).sort((a,b)=>(Number(a.seq)||0)-(Number(b.seq)||0));ts.forEach((t,i)=>{t.seq=i+1;t.dependsPrev=i>0;t.sequenceLocked=true})}}
...
function allocateInternal(t,earliestAt,opts={}){
 ...
 let d=dtDate(earliestAt),firstDay=true,segs=[],guard=0;
 while(need>0&&guard++<240){let best=null;for(const emp of employees){for(const w of freeWindows(d,emp,machineKey(t),excludeIds,allowSaturday,allowPeter)){let a=w[0],z=w[1];if(firstDay)a=Math.max(a,min(dtTime(earliestAt)));const free=z-a;if(free<=0)continue;if(!best||a<best.a||(a===best.a&&free>best.free))best={emp,a,z,free}}}
   if(best){let take=Math.min(need,best.free);if(need>take&&take<MIN_USEFUL_BLOCK){d=addCal(d,1);firstDay=false;continue}segs.push({date:d,employee:best.emp,start:tm(best.a),minutes:take});need-=take;firstDay=false;if(need>0)d=addCal(d,1)}else{d=addCal(d,1);firstDay=false}
 }
...
function pairSetupWithExecution(setup,run,earliestAt,opts={}){
 const sd=Math.max(1,Number(setup.estimate)||30),key=machineKey(run)||machineKey(setup),exclude=new Set([setup.id,run.id]);let d=dtDate(earliestAt),guard=0;
 let employees=run.employee&&run.employee!=='Ralph'?[run.employee]:['Kaan','Lance','Shaffi'];if(opts.allowPeter)employees.push('Peter');
 while(guard++<180){const rw=freeWindows(d,'Ralph',key,exclude,!!opts.allowSaturday,!!opts.allowPeter);for(const emp of employees){const ew=freeWindows(d,emp,key,exclude,!!opts.allowSaturday,!!opts.allowPeter);for(const e of ew){let runStart=Math.max(e[0],min(dtTime(earliestAt))*(d===dtDate(earliestAt)?1:0));if(d!==dtDate(earliestAt))runStart=e[0];for(const r of rw){const candidate=Math.max(runStart,r[0]+sd);if(candidate<e[1]&&candidate<=r[1]&&candidate-sd>=r[0]){
           setup.planSegments=[{date:d,employee:'Ralph',start:tm(candidate-sd),minutes:sd}];setup.employee='Ralph';setup.date=d;setup.start=tm(candidate-sd);setup.preferredEmployee='Ralph';
...
function clearMovablePlanning(orderIds=null){const ids=orderIds?new Set(orderIds):null;for(const t of S()?.tasks||[]){if(isGeneral(t)||taskDone(t)||frozen(t)||(ids&&!ids.has(t.orderId)))continue;clearTaskPlanning(t);if(isExternalTask(t)&&t.status!=='external'){t.externalSentDate='';t.expectedReturnDate=''}}}
...
function health(o){setDerivedDates(o);const finish=finishDate(o),target=targetDate(o),hard=hardDate(o),slack=hard?workdaySlack(finish,hard):null;let status='ok',label='Ruim haalbaar';if(!hard){status='risk';label='Geen maximumdatum'}else if(finish>hard){status='bad';label='Niet haalbaar'}else if(target&&finish>target){status='risk';label='In bufferzone'}else if(slack!==null&&slack<=2){status='risk';label='Weinig speling'}return {finish,target,hard,slack,status,label}}
...
function changedTasks(before,after){const map=new Map((before.tasks||[]).map(t=>[t.id,t]));const out=[];for(const t of after.tasks||[]){const b=map.get(t.id);if(!b||isGeneral(t)||taskDone(t))continue;const a1=JSON.stringify(b.planSegments||[]),a2=JSON.stringify(t.planSegments||[]);if(a1!==a2){const o=(after.orders||[]).find(x=>x.id===t.orderId);out.push({orderNo:o?.orderNo||'',product:o?.product||'',task:t.name,from:(b.planSegments||[])[0]?.date||'',to:(t.planSegments||[])[0]?.date||''})}}return out}
...
function enforceManualMove(t){if(!t||isGeneral(t))return;normalizeSequences();const ts=orderTasks(t.orderId).sort((a,b)=>a.seq-b.seq),idx=ts.findIndex(x=>x.id===t.id);if(idx<0)return;let cursor=taskFinishAt(t);for(let i=idx+1;i<ts.length;i++){const x=ts[i];if(taskDone(x)||taskStarted(x)||x.lockedPlanning)break;if(!cursor)break;if(isDryTask(x)){scheduleDryTask(x,cursor);cursor=x.waitEndAt}else if(isExternalTask(x)){const d=dtDate(cursor),lead=Number(x.externalLeadDays)||14;x.employee=null;x.planSegments=[];x.date=d;x.expectedReturnDate=addCal(d,lead);cursor=dtString(x.expectedReturnDate,'00:00')}else{const emp=isSetup(x)?'Ralph':(x.employee||x.preferredEmployee||bestEmployeeForTask(x,cursor,false));cursor=allocateInternal(x,cursor,{allowPeter:emp==='Peter'})}}
}
function install(){if(typeof window.renderOrders!=='function'||typeof window.openOrder!=='function')return setTimeout(install,300);normalizeSequences();for(const o of S()?.orders||[])setDerivedDates(o);save();
