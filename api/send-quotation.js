module.exports=async function handler(req,res){
  ...
  const sender=process.env.MS_SENDER_EMAIL||'ralabaster@ralabaster.onmicrosoft.com';
  const replyTo=process.env.MS_REPLY_TO_EMAIL||'info@ralabaster.com';
  ...
  try{
    ...
    const mail={message:{subject,body:{contentType:'Text',content:text},toRecipients:[{emailAddress:{address:to}}],replyTo:[{emailAddress:{name:'rAlabaster BV',address:replyTo}}]},saveToSentItems:true};
    ...
    return res.status(200).json({ok:true,sender,replyTo,to,quotationNo:quotationNo||''});
